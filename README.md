...
> 把安卓手机变成蓝牙键盘：横屏**全屏键盘** + **电脑全键盘** + **双手分体**三种布局，经蓝牙 HID 直连 iPad 等主机直接打字。
> 支持**文字转发（边打边发）**：在键盘页的文本框里用你自己的输入法打字，字符边打边发到主机。
> 也可作为**输入法**使用（手机负责录入，按键流发给主机）。**无需 root。**

Turn your Android phone into a Bluetooth HID keyboard — three layouts (full-screen landscape,
full desktop-style, and a split two-thumb layout), plus on-the-fly text forwarding and an
input-method mode that forwards typed text to the host. No root required.

---

## 状态：可用（全屏键盘 + 3 套布局 + 文字转发 + 连接管理 + 输入法）
...
| **3 套布局** | 电脑全键盘（60%，15 格宽）· 全屏（带数字行，12 格宽）· 双手分体（横屏，12 格宽：中间留空、左右手各一半、两个空格条）；**选择会被记住**，从未选择过时：竖屏（窄窗口）默认全屏（带数字行），横屏（宽窗口）默认电脑全键盘 |
